...
- [Make the call](#make-the-call) - Make a phone call your application and walk through the prompts to hear the spoken information.
...
- [Code review: Request user input via IVR](#request-user-input-via-ivr) - Create a text-to-speech prompt followed by requesting user input.
...
* The tutorial code from <https://github.com/Nexmo/php-phone-menu>, either clone the repository or download and extract the zip file to your machine.
...
The `Menu` class has a few more methods that are there to improve the process of turning application data in to spoken prompts. Examples in this application include:

* the date of when the status was last reported
* the order number
* the order status
